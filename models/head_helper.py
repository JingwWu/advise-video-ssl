...
class ResNetBasicHead(nn.Module):
    ...
    def __init__(
        ...
        if cfg.SSL.NUM_MLP_LAYERS == 1:
            ...
            self.projection = MLPHead(
                ...
                num_classes,
                cfg.SSL.MLP_DIM,
                cfg.SSL.NUM_MLP_LAYERS,
                bn_on=cfg.SSL.BN_MLP,
                bn_sync_num=cfg.BN.NUM_SYNC_DEVICES
                if cfg.SSL.BN_SYNC_MLP
                else 1,
                global_sync=(
                    cfg.SSL.BN_SYNC_MLP and cfg.BN.GLOBAL_SYNC
                ),
            )
        ...
        if hasattr(cfg, 'CONTRASTIVE') and cfg.CONTRASTIVE.PREDICTOR_DEPTHS:
            d_in = num_classes
    ...
    def forward(self, inputs):
        ...
        if (
            ...
            and (
                self.cfg.MODEL.MODEL_NAME == "ContrastiveModel"
                or
                self.cfg.TASK in [
                    "speed", "action_recog", "tmodeling"
                ]
            )
        ):
